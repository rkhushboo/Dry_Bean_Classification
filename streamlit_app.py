...
def create_sample_data():
    ...
    classes = ['SIRA', 'HOROZ', 'CALIPH', 'BOMBAY', 'DERMASON', 'CANKIRI', 'MUNG']
...
def home_page():
    ...
    st.subheader("🫘 Bean Varieties")
    bean_varieties = ['SIRA', 'HOROZ', 'CALIPH', 'BOMBAY', 'DERMASON', 'CANKIRI', 'MUNG']
    cols = st.columns(len(bean_varieties))
    for idx, bean in enumerate(bean_varieties):
        with cols[idx]:
            st.info(f"🫘 {bean}")
...
def about_page():
    ...
    with col1:
        ...
        st.write("""
        **Project By:** IIT Guwahati Data Science Team
